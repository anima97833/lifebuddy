...
【用户的个人生活数据（以下是真实完整数据，是你回答问题的唯一依据）】：
${lines.join('\n')}

【强制回答规则】：
当用户询问某个模块的数据（如收藏分布、待办、仪式等）时，你必须：
1. 第一句话直接回答该模块的内容。
2. 原原本本列出上述数据中的具体数字和名称。
3. 绝不可以说"我是机器人..."或"收到指令"等废话。直接切入正题，例如："我看到你的收藏分布有..."。`;
    }
  }
...
【注意】：当前用户没有保存任何数据记录，或者未能获取到数据。如果用户问及他们的记录，请如实回答："我目前没有看到你记录的数据哦，可能你还没有添加，或者数据还没同步过来。"`;
  }
...
【回复格式指令 — 必须严格遵守】：
1. 每次回复必须拆分为 3 条独立的简短消息。
2. 绝对不能使用列表格式，只用自然语言对话。
3. 每条消息之间使用 \`|||\` 作为分隔符。
例如：好的，我帮你看了下|||你的收藏分布里有一个「游戏」节点，总共花了1.94元|||里面包含了设计课程、笔刷和字体哦
请严格按此格式返回。`;
}
